defmodule Snitch.Data.Model.CountryZone do
  ...
  @doc """
  Creates a new state `Zone` whose members are `country_ids`.
  ...
  def member_ids(zone_id) do
    query = from(s in CountryZoneMember, where: s.zone_id == ^zone_id, select: s.country_id)
    Repo.all(query)
  end
